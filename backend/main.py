...
try:
    if not catalog_service.list_products():
        ...
        p2 = Product(id="2", name="Sweat Sarcastique", description="Molleton doux", price_cents=6000, stock_qty=8)
        p3 = Product(id="3", name="Casquette Stylée", description="Bleu marine, taille unique", price_cents=2000, stock_qty=20)
        p4 = Product(id="4", name="Mug Caféiné", description="Céramique blanche 30cl", price_cents=1500, stock_qty=15)
        products.add(p1)
        products.add(p2)
        products.add(p3)
        products.add(p4)
